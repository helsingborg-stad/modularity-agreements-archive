            ...
            <div className="box box-panel box-panel-secondary">
                ...
                <table className="table table-striped" items={paginatedItems}>
                    ...
                    {paginatedItems.map((item, i) => (
                        (item.Supplier.Name) ?
                            <tr key={'row'+item.Id}>
                                <td key={'supplierName_'+item.Id}><a href={""}>{item.Supplier.Name}</a></td>
                                <td key={'category_'+item.Id}>{item.Category}</td>
                                <td className="hidden-xs" key={'number_'+item.Id}>{item.Number}</td>
                                <td className="hidden-xs" key={'validFrom_'+item.Id}>{item.ValidFrom.replace('T00:00:00','')}</td>
                                <td className="hidden-xs" key={'validTo_'+item.Id}>{item.ValidTo.replace('T00:00:00','')}</td>
